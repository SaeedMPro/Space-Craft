    ...
    for (const auto& direction : directions) {
        ...
        if (isValidPosition(newX, newY, currentMap) && !visited[newX][newY]) {
            ...
            case '0':
                ...
                break;
            case '3':
                temp = orbit(currentMap->spaceObject, current, {newX, newY});
                break;
            case '1':
                logDecision("Moving to (" + to_string(newX) + ", " + to_string(newY) + ")");
...
Cardinal SpaceCraft::orbit(SpaceObject so, Cardinal start, Cardinal pos)
{
    ...
    // routing
    if (start.x < x1) { start.x += 2; }
    else if (start.x > x2) { start.x -= 2; }
    else if(start.y < y1) { start.y += 2; }
    else if(start.y > y2) { start.y -= 2; }
...
{
    for (auto &&spaceCurrent : sc)
    {
        if (start.x == spaceCurrent.pos1.x && start.y == spaceCurrent.pos1.y)
        {
            return spaceCurrent.pos2;
        }
        else if (start.x == spaceCurrent.pos2.x && start.y == spaceCurrent.pos2.y)
        {
            return spaceCurrent.pos1;
        }  
    }
